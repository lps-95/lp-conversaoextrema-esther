          ...
          <MSection>
            <div className='group relative'>
              ...
              <form onSubmit={handleSubmit} className='relative space-y-6 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-8 sm:p-10 shadow-2xl'>
                ...
                {status === 'success' && (
                  <div className='text-green-500 text-sm text-center'>Lead enviado com sucesso! Aguarde o WhatsApp abrir.</div>
                )}

                <button
                  type='submit'
                  className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-bold text-base bg-gradient-to-r from-button-primary to-accent-gold text-primary-dark shadow-xl hover:scale-[1.02] transition-transform duration-150 focus:outline-none focus:ring-2 focus:ring-button-primary/50 ${status === 'loading' ? 'opacity-60 cursor-not-allowed' : ''}`}
                ...
                >
                  {status === 'loading' ? (
                    ...
                    <span className='inline-flex items-center gap-2'>
                      <span className='text-lg'>4ac</span> Enviar e Avançar no WhatsApp
                    </span>
                  )}
                </button>
