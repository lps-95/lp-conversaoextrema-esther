import { caseContent } from '../../content/case'
import { useLanguage } from '../../contexts/LanguageContext'
import ParallaxLayer from '../ParallaxLayer'

export default function Case() {
  const { language } = useLanguage()
  const content = caseContent[language]
        ...
        <div className='group relative'>
          ...
          <div className='relative bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-8 sm:p-12'>
            ...
            <div className='flex items-center gap-4'>
              ...
              <div>
                <p className='font-bold text-xl text-text-primary'>{content.name}</p>
                <p className='text-text-secondary'>{content.role}</p>
                <div className='flex items-center gap-2 mt-1'>
                  <span className='text-button-primary text-sm font-bold'>{content.revenue}</span>
                  <span className='text-text-tertiary'>•</span>
                  <span className='text-green-400 text-sm font-bold'>{content.growth}</span>
                </div>
              </div>
            </div>
          </div>
